...
There is a :code:`title_screen` and :code:`pause_menu` preset added by default:
...
You can set a nine slice sprite four your buttons background, to set it go to :code:`item_look` > :code:`button` and set the :code:`nine_slice`
variable to a sprite that is nine slice enabled.
